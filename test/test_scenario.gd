...
func test_every_unit_has_both_animation_sheets() -> void:
	var grid := Scenario.build_grid()

	for unit in Scenario.populate(grid):
		for path in [unit.data.sprite_walk, unit.data.sprite_slash]:
			assert_ne(path, "", "%s is missing an animation sheet" % unit.data.unit_name)
			assert_true(ResourceLoader.exists(path), "%s points at a missing sheet: %s" % [unit.data.unit_name, path])

func test_no_two_units_share_a_look() -> void:
	var grid := Scenario.build_grid()
	var seen: Dictionary[String, bool] = {}

	for unit in Scenario.populate(grid):
		assert_false(seen.has(unit.data.sprite_walk), "%s reuses another unit's art" % unit.data.unit_name)
		seen[unit.data.sprite_walk] = true
