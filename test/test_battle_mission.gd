...
func test_battle_parameterized_with_m01_spawns_mission_squads() -> void:
	...
	assert_eq(battle._turns.units_awaiting_orders().size(), 4, "turn order sees the populated player roster immediately")
	...
	assert_eq(grid.unit_at(Vector2i(1, 5)).data.unit_name, "Raider")
